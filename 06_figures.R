...
# Read shape files
df_l1_sf <- read_sf(
  "//files.drexel.edu/colleges/SOPH/Shared/UHC/Projects/Wellcome_Trust/Data Methods Core/Geodatabases/SALURBAL/Shapefiles/SALURBAL_L1.shp"
) |>
...
fig_2_plots <- map2(fig_2_cities, fig_2_titles, \(ID, title) {
  ...
  plot_RR <- list_blup_pred$all_ages_deaths[[ID]] |>
    ...
    scale_y_continuous(limits = c(.75, 3.5), breaks = c(0.8, 1, 1.25, 1.5, 2)) +
    scale_x_continuous(limits = x_lims) +
    coord_trans(y = "log") +
    scale_color_manual(
      values = c("Hot temperatures" = "red", "Cold temperatures" = "blue")
    ) +
    labs(
      title = title,
      subtitle = glue("Mean temperature {mean_temp}°C"),
      color = ""
    ) +
    theme(
...
# Combine shape and temperature information
df_l1_slope <- right_join(
  df_l1_sf,
  df_RR_slope,
  by = join_by(SALID1 == nsalid1)
)
